...
int inserirPosicao(BigInt *l, int it, int pos){
    if(l == NULL) return 1;
    if(listaVazia(l) == 0) return inserirInicio(l, it);
    if(pos == 0) return inserirInicio(l, it);
    if(tamanho(l) <= pos) return inserirFim(l,it);
...
int removerFim(BigInt *l){
    if(l == NULL) return 1;
    if(listaVazia(l) == 0) return 2;
    if(tamanho(l) == 1) removerInicio(l);
...
int removerPosicao(BigInt *l, int pos){
    ...
    if(tamanho(l) - 1 <= pos) return removerFim(l);
...
int converteSinal(BigInt *l){
    ...
    while (n->prox != NULL){//vai até o último número da lista
        n = n->prox;
    }

    if(n->valor < 0){//se o digito mais significativo é negativo
        while (n != NULL){
            if(n->valor > 0) n->valor = n->valor * (-1); //se o número é positivo, vira negativo e n passa pro anterior
            n = n->ant;
        }
    }
...
int removeZero(BigInt *l){
    ...
    while (n != NULL && tamanho(l) != 1){
        if(n->valor == 0){ //se for zero, vai pro digito anterior e o remove
            n = n->ant; 
            removerFim(l);
        } else { //se não, ele para
            return 0;
        }
    }
...
int soma(BigInt *l1, BigInt *l2, BigInt *l3){
    ...
    if (n == NULL && m == NULL && r != 0) inserirFim(l3,r); //Se todos os digítos foram somados e ainda existe resto, insere o resto
...
int subtracao(BigInt *l1, BigInt *l2, BigInt *l3){
    ...
    while (n != NULL && m != NULL)
    {
        if(n->valor >= m->valor || n->prox == NULL) s = n->valor - m->valor; //se o dígito for maior igual do que o dígito do subtraendo, então s é o digíto menos o outro
            ...
            while(extra->ant != n){ //passa pelos anteriores somando 10 e subtraindo 1 até chegar no digíto original
                extra = extra->ant;
    ...
    while(n != NULL){ //caso n seja maior que m, insere os digítos que faltam
...
BigInt *maior(BigInt *l1, BigInt *l2){
    if(l1 != NULL && l2 != NULL){
        if(tamanho(l1) > tamanho(l2)) return l1; //se o tamanho de uma das listas for maior, então essa é a lista maior
        if(tamanho(l1) < tamanho(l2)) return l2;

        No *n = l1->inicio;
        No *m = l2->inicio;

        while (n->prox != NULL && m->prox != NULL){ // vai até o último digíto
            m = m->prox; 
            n = n->prox;
        }

        while(n != NULL && m != NULL){
            if(n->valor > m->valor) return l1; //Compara cada digíto até que ache um digíto maior que o outro, assim retornando a lista com o dígito maior
            if(n->valor < m->valor) return l2;
